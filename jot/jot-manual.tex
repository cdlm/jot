...
\begin{table}\centering
    \begin{tabularx}{\linewidth-2cm}{llX}
        ...
        \\
        \code{doisuffix} & string & Paper-specific part of the \caps{DOI} identifier. \\
        \code{doi} & string & Full \caps{DOI} identifier (without resolver \caps{URL}). \\
...
In the bibliography, use the |alpha| style for reference keys:
\begin{lstlisting}
    \bibliographystyle{alpha}
\end{lstlisting}
...
For more info on supported languages or how to highlight parts of code, refer to the documentation of |listings|; please do not abuse of text styles though, most often the default font is really enough.\footnote{Highlighting is based on visual contrast and thus relies on scarcity to be effective.}
...
\begin{itemize}
    \item do not specify vertical rules in tables, and forget about |\hline|;
    \item use |\toprule|, |\bottomrule|, and just a few |\middlerule| in between;
