...
\def\parambegin{\rmfamily\textlangle\itshape}
\def\paramend{\upshape\textrangle}
\def\parameter#1{{\parambegin#1\paramend}}
\def\code#1{\texttt{#1}}
\let\file\code
...
\begin{abstract}
    This short manual documents the \jotcls \LaTeX{} document class, and provides guidelines and advice on how to use it to prepare and typeset article manuscripts for submission to \caps{jot}, the Journal of Object Technology.
\end{abstract}
...
{\centering
    \begin{tabular}{l@{\qquad}l@{\qquad}l}
        ...
        \code{ifthen}   & \code{refcount} & \code{xcolor} \\
        \code{calc}     & \code{soul}     & \code{eso-pic} \\
        \code{geometry} & \code{ragged2e} & \code{placeins} \\
                        & \code{wrapfig}
    \end{tabular}\par}
...
The |\jotdetails| declaration defines publication and indexing information about the article, in |key=value| form.
\begin{lstlisting}
...
{\centering
    \begin{tabularx}{\linewidth-2cm}{llX}
        ...
        \code{url} & \caps{url} & Address of the paper in the \caps{jot} web site. \\
        \bottomrule
    \end{tabularx}\par}
...
When the figure or the table is quite large, however, you should force it to be as wide as the text, by scaling the graphics or using the |tabularx| package:

\begin{lstlisting}
    \includegraphics[width=\linewidth]{_$\cdots$_}
    \begin{tabularx}{\linewidth}{_$\cdots$_}
        _see the tabularx documentation for the X column specifier_
    \end{tabularx}
\end{lstlisting}

If a figure or a table really requires lots of horizontal space, you can use the |fullwidth| environment to temporarily reduce the margins, as shown in Figure~\ref{fig:fullwidth}.
\begin{figure}
